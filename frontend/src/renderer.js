...
canvas.onclick = function (e) {
    ...
    const y = Math.floor((e.y - rc.top + rc.cell / 2) / rc.cell);
    if (x > 8 || y > 9)
        return;
    const px = rc.left + x * rc.cell;
    const py = rc.top + y * rc.cell;
    const dist = distance({x: px, y: py}, {x: e.x, y: e.y});
    if (dist <= radius) {
        const v = gameState.board[y * 9 + x];
        console.log("getColor", getColor(v), player, setting.color);
        if (getColor(v) === 'r' && player === "red" && setting.color === player) {
            ...
            console.log("activePos", activePos, x, y);
